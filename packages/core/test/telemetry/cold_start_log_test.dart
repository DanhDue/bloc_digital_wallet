...
void main() {
  group('ColdStartLog Tests', () {
    test('has title COLD START and key cold_start', () {
      final log = ColdStartLog('test cold start message');

      expect(log, isA<TalkerLog>());
      expect(log.title, equals('COLD START'));
      expect(log.key, equals('cold_start'));
      expect(ColdStartLog.getKey, equals('cold_start'));
