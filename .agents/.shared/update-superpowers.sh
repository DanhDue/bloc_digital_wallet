...
VERSION_FILE=".agents/superpowers-version.json"
...
echo "📋 Syncing .agents/agents/ from superpowers/agents/..."
...
for agent_src in superpowers/agents/*.md; do
  ...
  #   .agents/  →  .agents/   (superpowers uses .agents/, Antigravity uses .agents/)
  cp "$agent_src" "$dest"
  sed -i '' 's|\.agents/|.agents/|g' "$dest"
...
echo "🔄 Syncing .agents/workflows/ from superpowers/commands/..."
