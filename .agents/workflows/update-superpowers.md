---
description: Update superpowers to the latest GitHub release and sync .agents/ files
---
...
1. Preserve user config (if exists):
   `mkdir -p .agents/tmp && [ -f .agents/config.yml ] && cp .agents/config.yml .agents/tmp/agent-config-backup.yml && echo "Config backed up" || echo "No config to backup"`

// turbo
2. Run the update script:
   `bash .agents/.shared/update-superpowers.sh`

   - If output ends with "Already up to date" → restore config if backed up (`[ -f .agents/tmp/agent-config-backup.yml ] && cp .agents/tmp/agent-config-backup.yml .agents/config.yml`), then **STOP**. Nothing to do.
...
4. Restore user config:
   `[ -f .agents/tmp/agent-config-backup.yml ] && cp .agents/tmp/agent-config-backup.yml .agents/config.yml && echo "Config restored" || echo "No config to restore"`

5. **Phase 2 — Update skill list in rules**

   List all skill folders now present in `.agents/skills/`.
   Open `.agents/rules/superpowers.md` and update **only the skills table**:
...
6. **Phase 2 — New skills check**
   ...
   For any skill in `.agents/skills/` that has no corresponding workflow in `.agents/workflows/`:
...
7. **Phase 2 — Apply skill patches from `.agents/patches/skills-patches.md`**

   Read `.agents/patches/skills-patches.md`. For each patch entry:
   - Open the target SKILL.md (path relative to `.agents/skills/`)
...
8. Commit all changes from Phase 2:
   `git add .agents/ && git commit -m "chore: sync .agents/ with superpowers <new-tag>"`
