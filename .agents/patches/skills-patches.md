...
Antigravity block: skills are read using `view_file` on `.agents/skills/<name>/SKILL.md`.
...
`~/.agents/skills/` for Codex) with the Antigravity path: `~/.agents/skills`.
...
**Intent:** Make the step that commits the design document to git conditional on `.agents/config.yml`:
- Read `.agents/config.yml` before committing
...
**Intent:** Make the implementer subagent commit step conditional on `.agents/config.yml`:
- Read `.agents/config.yml` before committing
- If `auto_commit: true` (default): commit normally with `git add` + `git commit`
- If `auto_commit: false`: skip commit and staging entirely. Print: "Skipping commit (auto_commit: false in .agents/config.yml). Files left as modified for manual commit."
...
**Intent:** Make the commit step in the plan task template conditional on `.agents/config.yml`:
- Step 5 should be titled "Commit (if auto_commit enabled)" instead of just "Commit"
- Before the `git add`/`git commit` block, add: Check `.agents/config.yml` for `auto_commit` setting
...
- Add sub-steps: read `.agents/config.yml`, if `auto_commit: true` (or not set): `git add` + `git commit`, if `auto_commit: false`: skip commit and staging entirely, print skip message.
...
- Read `.agents/config.yml` before the git operation
...
- Step 2 (commit): check `auto_commit` in `.agents/config.yml`
...
**Intent:** Replace all `/tmp/` paths with `.agents/tmp/` equivalents. Temp data should stay
within the project's `.agents/tmp/` directory instead of polluting the system `/tmp/`.

Specific changes:
- `server.cjs`: fallback `SESSION_DIR` should resolve to `.agents/tmp/brainstorm` via `path.resolve(__dirname, '..', '..', '..', 'tmp', 'brainstorm')`
- `start-server.sh`: compute `AGENT_DIR` from `SCRIPT_DIR` (walk up 3 levels to `.agents/`), use `${AGENT_DIR}/tmp/brainstorm-${SESSION_ID}` as fallback
- `stop-server.sh`: cleanup condition should match both `/tmp/*` and `*/.agents/tmp/*`
- `visual-companion.md`: documentation references to `/tmp` → `.agents/tmp/`
- `testing-skills-with-subagents.md`: example path `/tmp/payment-system` → `.agents/tmp/payment-system`
- Rust reference docs: all `/tmp/` example paths → `.agents/tmp/` equivalents
