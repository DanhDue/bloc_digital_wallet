---
description: File length policy — apply when creating or editing files in .agents/rules/
alwaysApply: true
priority: critical
---
...
<HARD-GATE>
Do NOT write, append, or edit any file in `.agents/rules/` until you have checked its current character count.
If adding content would push the file past 12,000 characters, you MUST create a new file instead.
This gate applies exclusively to `.agents/rules/` files.
</HARD-GATE>

# File Length Policy

> **Core rule:** Before writing to any `.agents/rules/` file, check its size. If adding content would exceed 12,000 characters total, create a new file instead.

## Hard Limit: 12,000 Characters Per File

Every file in `.agents/rules/` has a **hard character limit of 12,000 characters**.

Before writing or appending to any `.agents/rules/` file, check the current file size:
