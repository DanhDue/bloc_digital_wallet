...
> **Core rule:** NEVER use `/tmp/` — always use `.agents/tmp/` and set `Cwd` to workspace root. `/tmp/` causes `run_command` to hang forever.

> **PRIORITY: CRITICAL** — Violating this rule causes commands to hang and freeze the conversation.

## The Rule

**NEVER create or run scratch scripts in `/tmp/`.** Always use `.agents/tmp/` instead.
...
1. **Path**: All scratch/temporary scripts go in `.agents/tmp/`, NOT `/tmp/`
2. **Cwd**: When calling `run_command`, set `Cwd` to the **workspace root** — never to `/tmp/` or any path outside the workspace
3. **Create dir first**: Run `mkdir -p .agents/tmp` before writing scratch files if the directory might not exist
...
# ✅ CORRECT
Cwd: /path/to/project
CommandLine: node .agents/tmp/test-parser.js
