...
- `bricks/pac_mvi_feature/hooks/post_gen.dart` — `_updateFeaturePublicRoutes` function, retarget its file path.
- `bricks/pac_mvi_feature/__brick__/packages/{{name.snakeCase()}}/pubspec.yaml` — add `platform: {path: ../platform}` alongside the existing `core`/`network`/`ui_kit`/`framework` local dependencies.
...
1. Run `mason make pac_mvi_feature` with a scratch package name in a throwaway location; confirm the generated `pubspec.yaml` includes the `platform` dependency and the `post_gen.dart` hook writes the route entry into `packages/platform/lib/deep_link_routes.dart` (not the old `core` path).
2. Run `mason make remove_pac_feature` against that same scratch package; confirm its `platform` entry is cleaned up.
...
- [ ] A freshly-generated `pac_mvi_feature` package depends on `platform` and registers its route in `packages/platform/lib/deep_link_routes.dart`.
- [ ] `remove_pac_feature` correctly cleans up the `platform` entry it added.
