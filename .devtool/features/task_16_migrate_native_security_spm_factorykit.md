...
Migrate the FFI + Datadog-appender plugin `native_security` to a SwiftPM package, applying the pattern proven in Task 15 plus the FFI recipe from the Task 14 spike. **If Task 14 returned no-go, follow its fallback:** keep `native_security` on `.podspec` (hybrid tolerates it), record it for Phase 2, and reduce this task to "confirm hybrid build + no regression".
...
3. **FFI reachability**: retain `__attribute__((used)) __attribute__((visibility("default")))` in `native_security.h` and the `_ = get_ssl_pin_1() …` force-reference in `NativeSecurityPlugin.register(with:)`. Apply any `lib/native_security.dart` loader change the spike identified.
