...
- [ ] Grep `.agent/` for `bloc_digital_wallet` and replace per the coordination decision above.
- [ ] Reset `.agent/contexts/*.md` to empty/generic templates.
...
- [ ] `grep -r bloc_digital_wallet .agent/` returns no project-specific hits requiring a rename-script pass (or all remaining hits are confirmed to be handled by Task 7).
