...
Relevant project skill: `test-driven-development` (`.agents/skills/test-driven-development`) — the dispatch matrix (module × appender toggle combinations) is exactly the kind of branching logic that skill's test-first process is meant to pin down before implementation.
