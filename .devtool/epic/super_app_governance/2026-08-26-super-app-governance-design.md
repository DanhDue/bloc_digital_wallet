...
- `pac_mvi_feature/__brick__/.../pubspec.yaml`: add `platform: {path: ../platform}` alongside the existing `core`/`network`/`ui_kit`/`framework` local dependencies.
...
**Phase 1 — Pilot: `settings`.** `onboard`'s import of `settings` is a genuine push-navigation (post-onboarding hand-off) — migrate it off the direct import onto `DeepLinkRoutes.settingsRoute` navigation (and `AppEventBus` if any settings-driven cross-feature signal is needed), then remove the `onboard→settings` entry from the whitelist and verify CI now hard-blocks a deliberately reintroduced direct import. `home`'s import of `settings` is a different shape — `home_page.dart` *embeds* `SettingsPage()` as one of five `IndexedStack` tab children, not a pushed route, so `DeepLinkRoutes` doesn't apply to it. `home→settings` stays whitelisted through Phase 1 and is resolved together with `home`'s other four feature imports in Phase 2, since all five are the same kind of embed and belong together in one atomic move rather than being split across phases.
...
**Phase 3 — Remaining packages.** With the coupling violations resolved by Phase 1–2, this phase is about hardening: audit all 7 feature packages' barrels for the export-discipline rule, tighten the CI Gate's deep-import check package by package, and remove the (by-then-empty) whitelist file.
