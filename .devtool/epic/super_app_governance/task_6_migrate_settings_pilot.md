---
id: "task_6_migrate_settings_pilot"
status: "done"
priority: "high"
assignee: null
...
order: "a6"
---
# Task 6: Migrate pilot package `settings` (`onboard→settings`) — REDEFINED
...
2. Confirm the pilot's actual validation goal — proving `DeepLinkRoutes` works end-to-end before Tasks 7/8 rely on it — was already satisfied by Task 1's own `splash_page.dart` → `DeepLinkRoutes.homeRoute` migration (implemented and reviewed as part of Task 1).
...
4. Update this epic's spec/HLD Migration Plan and Task 8 to reflect that the whitelist will NOT end up empty at the end of this epic — one entry (`onboard→settings`) remains by design.
...
- `.devtool/features/task_8_hardening_barrel_audit.md` — updated (separately) to account for the whitelist not reaching zero entries.
...
- [x] Epic spec/HLD Migration Plan and Task 8 updated to reflect the whitelist will not reach zero entries within this epic.
...
Blocked by [Task 1](task_1_create_platform_package.md) (needed `DeepLinkRoutes` to evaluate whether it fit — it didn't, for this coupling) and [Task 3](task_3_ci_module_boundary_gate.md) (needed the whitelist/gate to confirm the real coupling's exact location).
...
- Epic ledger: `.superpowers/sdd/super_app_governance.en/progress.md`, "Task 6: Ruling" — full investigation record.
