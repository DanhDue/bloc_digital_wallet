...
void main() {
  ...
  group('LoginBloc Tests', () {
    ...
    blocTest<LoginBloc, LoginState>(
      ...
      build: () {
        when(
          ...
          (_) => Future<Either<Failure, AuthUserEntity>>.value(const Right(tUser)),
        );
        return loginBloc;
      },
    ...
    blocTest<LoginBloc, LoginState>(
      ...
      build: () {
        when(
          mockUseCase.call(email: 'test@wallet.com', password: 'wrong_password'),
        ).thenAnswer(
    ...
    blocTest<LoginBloc, LoginState>(
      ...
      act: (bloc) => bloc.onAction(
        const LoginWithEmailPasswordAction(email: '', password: ''),
      ),
      expect: () => [
        const LoginLoading(),
        const LoginError('Email is required'),
      ],
    );
  });
}
