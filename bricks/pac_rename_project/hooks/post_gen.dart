...
Future<void> run(HookContext context) async {
  ...
  try {
    ...
    // 11c. Configure project mode (enterprise vs lean)
    progress.update('Configuring project mode to "$mode"...');
    final configureScript = File('${rootDir.path}/scripts/configure_mode.sh');
    if (configureScript.existsSync()) {
      final configResult = await Process.run(
          'bash',
          [
            'scripts/configure_mode.sh',
            mode,
            '--root-dir=${rootDir.path}',
            '--skip-verify',
          ],
          runInShell: true);
